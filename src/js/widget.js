(function (w, d) {
  ...
  const IFRAME_SELECTOR = 'iframe[name=donorbox]',
      DATA_ATTR = 'data-donorbox-id',
      UTM_PARAMS = extractUtmParams(),
      notBusyWithIDs = true;

  function extractUtmParams() {
    const data = {}, queryString = w.location.href.split('?')[1];
    ...
    if (queryString) {
      ...
      params.forEach(function(p) {
        const splitted = p.split('='), key = splitted[0], value = splitted[1];
  ...
  function onloadProcessing() {
    const iFrames = d.querySelectorAll(IFRAME_SELECTOR),
        apr = 'allowpaymentrequest';

    for (let i = 0; i < iFrames.length; i++) {
      let iframe = iFrames[i], src = iframe.src;
  ...
  if (d.querySelector('script[paypalExpress=true]')) {
    const paypal = d.createElement('script');
  ...
  w.donorbox = w['donorbox'] = {
    resizeDonationWidget: function(iFrameSelector) {
      const query = iFrameSelector || IFRAME_SELECTOR, el = iFrameSelector, iframe;
